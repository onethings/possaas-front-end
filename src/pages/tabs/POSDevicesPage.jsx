...
const POSDevicesPage = () => {
    ...
    const handleDelete = (idx) => {
        if (!window.confirm(t('pos_devices.confirm_delete', 'Confirm Delete'))) return;
        setDevices(prev => prev.filter((_, i) => i !== idx));
    };
    const handleSubmit = (e) => {
        e.preventDefault();
        const entry = { name: formData.name, status: t('pos_devices.enabled', 'Enabled') };
        if (editingIdx !== null) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '1.1rem' }}>{t('pos_devices.title', 'Title')}</h3>
                <button onClick={openCreate} className="btn-primary" style={{ padding: '0.5rem 1.2rem', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    <Plus size={16} /> {t('pos_devices.add', 'Add')}
                </button>
            </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                        <thead>
                            <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                                <th style={{ padding: '0.75rem 0.5rem', textAlign: 'left', color: 'var(--text-muted)' }}>{t('pos_devices.name', 'Name')}</th>
                                <th style={{ padding: '0.75rem 0.5rem', textAlign: 'left', color: 'var(--text-muted)' }}>{t('pos_devices.status', 'Status')}</th>
                        ...
                        <tbody>
                            ...
                            {devices.length === 0 && (
                                <tr><td colSpan={3} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>{t('common.no_data', 'No Data')}</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <h3>{editingIdx !== null ? t('pos_devices.edit', 'Edit') : t('pos_devices.add', 'Add')}</h3>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div style={{ marginBottom: '1rem' }}>
                                <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.3rem' }}>{t('pos_devices.name', 'Name')} *</label>
                            ...
                            <button type="submit" className="btn-primary" disabled={submitting}
                                ...
                                {submitting ? <Loader2 className="animate-spin" size={18} /> : (editingIdx !== null ? t('common.update', 'Update') : t('common.confirm_add', 'Confirm Add'))}
                            </button>
                        </form>
                    </motion.div>
                </div>
