...
const StoresPage = () => {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm(t('stores.confirm_delete', 'Confirm Delete'))) return;
        try {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '1.1rem' }}>{t('stores.title', 'Title')}</h3>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                                    <th style={{ padding: '0.75rem 0.5rem', textAlign: 'left', color: 'var(--text-muted)' }}>{t('stores.name', 'Name')}</th>
                                    <th style={{ padding: '0.75rem 0.5rem', textAlign: 'left', color: 'var(--text-muted)' }}>{t('stores.address', 'Address')}</th>
                                    <th style={{ padding: '0.75rem 0.5rem', textAlign: 'left', color: 'var(--text-muted)' }}>{t('stores.phone', 'Phone')}</th>
                                    <th style={{ padding: '0.75rem 0.5rem', textAlign: 'right', color: 'var(--text-muted)' }}>{t('stores.pos_count', 'Pos Count')}</th>
                            ...
                            <tbody>
                                ...
                                {stores.length === 0 && (
                                    <tr><td colSpan={5} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>{t('common.no_data', 'No Data')}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <h3>{editingStore ? t('stores.edit', 'Edit') : t('stores.add', 'Add')}</h3>
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div className="input-group">
                                <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('stores.name', 'Name')} *</label>
                            ...
                            <div className="input-group">
                                <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('stores.address', 'Address')}</label>
                                <input name="store-address" type="text"
                                    style={{ width: '100%', padding: '0.6rem', background: 'var(--input-bg)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-md)', color: 'var(--text-main)', outline: 'none' }}
                                    value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} />
                            </div>
                            <div className="input-group">
                                <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('stores.phone', 'Phone')}</label>
                            ...
                            <button type="submit" className="btn-primary" disabled={submitting}
                                ...
                                {submitting ? <Loader2 className="animate-spin" size={18} /> : (editingStore ? t('common.update', 'Update') : t('common.confirm_add', 'Confirm Add'))}
                            </button>
                        </form>
                    </motion.div>
                </div>
