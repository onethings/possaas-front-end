...
const LoyaltyPage = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <h3 style={{ fontSize: '1.1rem' }}>{t('loyalty.title', '客戶忠誠度計劃')}</h3>
            <div className="glass-panel" style={{ padding: '1.5rem', maxWidth: '500px' }}>
                <div style={{ marginBottom: '1.5rem' }}>
                    <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.5rem' }}>
                        {t('loyalty.earning_rate', 'Points earning percentage（點數賺取百分比）')}
                    </label>
                    ...
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.3rem' }}>
                        {t('loyalty.earning_rate_desc', '購買金額的百分比輸入客戶的積點帳戶')}
                    </p>
                </div>
