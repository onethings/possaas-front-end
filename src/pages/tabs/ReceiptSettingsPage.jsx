...
const ReceiptSettingsPage = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <h3 style={{ fontSize: '1.1rem' }}>{t('receipt_settings.title', 'Title')}</h3>

            {/* Logo Upload */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                <div className="glass-panel" style={{ padding: '1.5rem' }}>
                    <h4 style={{ fontSize: '0.9rem', marginBottom: '1rem', color: 'var(--text-muted)' }}>{t('receipt_settings.email_logo', 'Email Logo')}</h4>
                    <div style={{ width: '100%', height: '150px', border: '2px dashed var(--glass-border)', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                        ...
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>{t('receipt_settings.upload', 'Upload')}</span>
                    </div>
                </div>
                <div className="glass-panel" style={{ padding: '1.5rem' }}>
                    <h4 style={{ fontSize: '0.9rem', marginBottom: '1rem', color: 'var(--text-muted)' }}>{t('receipt_settings.print_logo', 'Print Logo')}</h4>
                    <div style={{ width: '100%', height: '150px', border: '2px dashed var(--glass-border)', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                        <Upload size={32} color="var(--text-muted)" />
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>{t('receipt_settings.upload', 'Upload')}</span>
                    </div>
            ...
            <div className="glass-panel" style={{ padding: '1.5rem' }}>
                <div style={{ marginBottom: '1.5rem' }}>
                    <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.5rem' }}>{t('receipt_settings.header', 'Header')} <span style={{ fontSize: '0.7rem' }}>{header.length} / 500</span></label>
                ...
                <div>
                    <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.5rem' }}>{t('receipt_settings.footer', 'Footer')} <span style={{ fontSize: '0.7rem' }}>{footer.length} / 500</span></label>
            ...
            <div className="glass-panel" style={{ padding: '1.5rem' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {[
                        { label: t('receipt_settings.show_customer', 'Show Customer'), key: 'showCustomer', checked: showCustomer, onChange: setShowCustomer },
                        { label: t('receipt_settings.show_notes', 'Show Notes'), key: 'showNotes', checked: showNotes, onChange: setShowNotes },
                    ].map((item) => (
            ...
            <div className="glass-panel" style={{ padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('receipt_settings.language', 'Language')}:</span>
