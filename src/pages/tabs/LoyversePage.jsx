...
const LoyversePage = () => {
    ...
    const runImport = async (importFn, label) => {
        ...
        try {
            ...
            if (res.taskId) {
                // Save taskId so it can be resumed on page refresh
                try { localStorage.setItem('loyverse_pending_task', JSON.stringify({ taskId: res.taskId, label, startedAt: Date.now() })); } catch (e) {}
                setLoyverseMessage({ type: 'info', text: `${label} 背景匯入中，請稍候...` });
                let attempts = 0;
                const maxAttempts = 900; // 15 minutes max (1s interval)
                const poll = async () => {
                    try {
                        ...
                        if (status.success && status.data) {
                            if (status.data.status === 'completed') {
                                ...
                                if (attempts < maxAttempts) {
                                    ...
                                    setLoyverseMessage({ type: 'info', text: `${label} 仍在背景執行中，可稍後重整頁面查看結果` });
                                    setLoyverseImporting(null);
                                    try { localStorage.removeItem('loyverse_pending_task'); } catch (e) {}
                                }
                            }
                        ...
                        if (attempts < maxAttempts) {
                            ...
                            setLoyverseMessage({ type: 'info', text: `${label} 已在背景啟動，可稍後重整頁面查看結果` });
                            setLoyverseImporting(null);
                            try { localStorage.removeItem('loyverse_pending_task'); } catch (e) {}
                        }
                    }
